...
class my_queue {
 ...
 public:
  ...
  my_queue() : head_(nullptr), tail_(nullptr) {}

  my_queue(const my_queue& rhs) : head_(nullptr), tail_(nullptr) {
  ...
  class iterator {
   ...
   public:
    ...
    bool operator==(const iterator& rhs) { return host_ == rhs.host_; }

    bool operator!=(const iterator& rhs) { return host_ != rhs.host_; }

    node* operator*() { return host_; }
  };
...
/*int main(int argc, const char* argv[]) {
  ...
  try {
    ...
    std::cout << "Exception test: " << ex.what() << std::endl;
  }
  ...
  if (argc > 2) {
    ...
    std::cout << "Write input file name\n";
    std::cin >> filename_in;
    std::cout << "Write output file name\n";
